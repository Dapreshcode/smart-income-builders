  ...
  import { incomeSystems } from "@/data/IncomeSystems";
  ...
  export default async function BlogPostPage({ params, searchParams }: any) {
    ...
    // ✅ RESOLVE LEARNING PATH CONTEXT
    // Prefer ?system=&guide= from the URL (so navigation stays on the
    // guide the reader is actually following); fall back to the post's
    // first learningPaths entry so direct visits / old links still work.
    const fallbackEntry = fm.learningPaths?.[0];
    const pathContext = {
      incomeSystem: resolvedSearchParams?.system ?? fallbackEntry?.incomeSystem ?? "",
      guide: resolvedSearchParams?.guide ?? fallbackEntry?.guide ?? "",
    };

    const sequence =
      pathContext.incomeSystem && pathContext.guide
        ? getPathSequence(slug, posts, pathContext, incomeSystems)
        : null;
                ...
                pathNavigator={
                  sequence && (
                    <ArticlePathNavigator
                      ...
                      next={sequence.next}
                      guideTitle={sequence.guideTitle}
                      phaseTitle={sequence.phaseTitle}
                      phaseComplete={sequence.phaseComplete}
                      nextGuide={sequence.nextGuide}
                      incomeSystem={pathContext.incomeSystem}
                      guide={pathContext.guide}
                      level={fm.level}
                    />
                  )
                }
